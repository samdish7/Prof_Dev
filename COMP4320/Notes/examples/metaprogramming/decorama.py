...
class deco_four():
    ...
    def __call__(self, wrapped_function):
        @wraps(wrapped_function)
        def _replacement(*args, **kwargs):
            fmt = "GREETINGS from deco_four -- value is {}!"
            print(fmt.format(self._value))
            result = wrapped_function(*args, **kwargs)
            return result + 4
...
def target_function(color, value):
    fmt = "Hello from target_function -- color is {} and value is {}"
    print(fmt.format(color, value))
...
def deco_six(fruit):
    ...
    returns wrapper applied to target class
...
class deco_seven():
    ...
    __new__() returns the modified class
    (not __init__, because __init__ is *instance* initializer)
